...
function main() {
    var incident = {
        // Incident name
        name: 'XTeVe connectivity issues',
        // Incident description (supports markdown)
        message: 'XTeVe is currently having connectivity issues on Firepower Cloud.',
        ...
        component_id: process.env.xteveid,
    ...
    cachet.reportIncident(incident)
        .then(function (response) {
            // Log API response
            console.log('XTeVe Partial Outage Reported at ' + response.data.created_at);
            (async () => {
