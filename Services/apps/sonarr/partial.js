...
function main() {
    var incident = {
        // Incident name
        name: 'Sonarr connectivity issues',
        // Incident description (supports markdown)
        message: 'Sonarr is currently having connectivity issues on Firepower Cloud.',
        ...
        component_id: process.env.sonarrid,
    ...
    cachet.reportIncident(incident)
        .then(function (response) {
            // Log API response
            console.log('Sonarr Partial Outage Reported at ' + response.data.created_at);
            (async () => {
